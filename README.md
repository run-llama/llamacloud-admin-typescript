...
```sh
npm install git+ssh://git@github.com:run-llama/llama-cloud-admin-ts.git
```
...
<!-- prettier-ignore -->
```ts
const page = await client.organizations.list().catch(async (err) => {
  if (err instanceof LlamaCloudAdmin.APIError) {
...
async function fetchAllOrganizations(params) {
  ...
  for await (const organization of client.organizations.list({ page_size: 20 })) {
...
```ts
let page = await client.organizations.list({ page_size: 20 });
...
const response = await client.organizations.list().asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: page, response: raw } = await client.organizations.list().withResponse();
...
1. Via the `LLAMA_CLOUD_ADMIN_LOG` environment variable
...
const client = new LlamaCloudAdmin({
  logger: logger.child({ name: 'LlamaCloudAdmin' }),
...
const client = new LlamaCloudAdmin({ fetch });
```
