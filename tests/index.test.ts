...
import { APIPromise } from '@llamaindex/llama-cloud-admin/core/api-promise';

import util from 'node:util';
import LlamaCloudAdmin from '@llamaindex/llama-cloud-admin';
import { APIUserAbortError } from '@llamaindex/llama-cloud-admin';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['LLAMA_CLOUD_ADMIN_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: LlamaCloudAdmin) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new LlamaCloudAdmin({ apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['LLAMA_CLOUD_ADMIN_LOG'] = 'debug';
      const client = new LlamaCloudAdmin({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['LLAMA_CLOUD_ADMIN_LOG'] = 'not a log level';
      const client = new LlamaCloudAdmin({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'LLAMA_CLOUD_ADMIN_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['LLAMA_CLOUD_ADMIN_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new LlamaCloudAdmin({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['LLAMA_CLOUD_ADMIN_BASE_URL'] = 'https://example.com/from_env';
      const client = new LlamaCloudAdmin({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['LLAMA_CLOUD_ADMIN_BASE_URL'] = ''; // empty
      const client = new LlamaCloudAdmin({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.cloud.llamaindex.ai');
    });

    test('blank env variable', () => {
      process.env['LLAMA_CLOUD_ADMIN_BASE_URL'] = '  '; // blank
      const client = new LlamaCloudAdmin({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.cloud.llamaindex.ai');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new LlamaCloudAdmin({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['LLAMA_CLOUD_ADMIN_BASE_URL'] = 'http://localhost:5000/env';
      const client = new LlamaCloudAdmin({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new LlamaCloudAdmin({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new LlamaCloudAdmin({ apiKey: 'My API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new LlamaCloudAdmin({ apiKey: 'My API Key', fetch: testFetch });
