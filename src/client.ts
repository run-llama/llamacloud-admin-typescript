...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['LLAMA_CLOUD_ADMIN_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['LLAMA_CLOUD_ADMIN_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Llama Cloud Admin API.
 */
export class LlamaCloudAdmin {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['LLAMA_CLOUD_ADMIN_BASE_URL'] ?? https://api.cloud.llamaindex.ai] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('LLAMA_CLOUD_ADMIN_BASE_URL'),
    apiKey = readEnv('LLAMA_CLOUD_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.LlamaCloudAdminError(
        "The LLAMA_CLOUD_API_KEY environment variable is missing or empty; either provide it, or instantiate the LlamaCloudAdmin client with an apiKey option, like new LlamaCloudAdmin({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? LlamaCloudAdmin.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('LLAMA_CLOUD_ADMIN_LOG'), "process.env['LLAMA_CLOUD_ADMIN_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('LLAMA_CLOUD_ADMIN_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  >(
    ...
    return new Pagination.PagePromise<PageClass, Item>(this as any as LlamaCloudAdmin, request, Page);
  }
  ...
  static LlamaCloudAdmin = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static LlamaCloudAdminError = Errors.LlamaCloudAdminError;
